...
#defines the function for seting the original directory vairable 
function setOriginalDir {
	CURRENT_DIR=$(pwd)
	if [[ $CURRENT_DIR == *"scripts"* ]]; then
		cd ..
	fi

	CURRENT_DIR_CONTENT=$(ls)

	if [[ $CURRENT_DIR_CONTENT == *"rodos"* ]]; then
		ORIGINAL_DIR=$(pwd)
	else
