...
function helpFunction {
	...
	echo -e "Make sure './setup.sh' has been run at least once and RODOS is compiled for Raspbian."
	echo -e "You can do that by running './setup.sh -c raspbian'."
	echo -e "\n"
    echo -e "\033[1mHow to use:\033[0m"
	echo -e "Run './build-for-raspbian.sh' to compile files from the rodos_src directory and execute them on your linux machine."
	echo -e "\n"
	echo -e "\033[1mExamples:\033[0m"
	echo -e "'./build-for-raspbian.sh'			compiles and executes all the files in rodos_src"
	echo -e "'./build-for-raspbian.sh HelloWorld.cpp' 	only compiles and executes HelloWorld.cpp"
	echo -e "'./build-for-raspbian.sh -f'			compiles and executes all the files specified in CompileList.txt"
	echo -e "\n"
...
function compileFiles {
	if [ "$SHOW_COMPILATION_OUTPUT" = true ]; then
		rodos-executable.sh on-posix $FILES_TO_COMPILE 2>&1 | tee -a CompilationLog
	else
		rodos-executable.sh on-posix $FILES_TO_COMPILE 2> CompilationLog
	fi
}
